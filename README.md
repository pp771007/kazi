# 咔滋影院 (kazi)

![咔滋影院](app/src/main/res/mipmap-xxxhdpi/ic_banner.webp)

原生 Android 影片聚合播放器，一次搜多站、聚合同名來源。手機 / 平板 / 電視盒（Android TV）都能裝。

## ⚠️ 免責聲明

- 本軟體是一個**通用的影片播放工具**，本身**不提供、不儲存、不內建任何影片內容或站台來源**。所有站台 URL 與影片內容皆由使用者自行提供與存取，與本軟體及作者無關。
- 本專案僅供**個人學習、技術研究與合法用途**。使用者須自行確保所存取的內容來源合法，並遵守所在地區的法律與著作權規範。
- 作者**不對**使用者透過本軟體存取的任何第三方內容之合法性、正確性或可用性負責；因使用本軟體所衍生的任何爭議與法律責任，**由使用者自行承擔**。
- 若任何內容涉及侵權，請逕向該內容的**來源站台**反映，與本軟體無關。
- **請勿**將本軟體用於任何違法用途。下載 / 安裝 / 使用本軟體，即視為您已閱讀並同意以上條款。

## 下載

直接抓 [最新 release](https://github.com/pp771007/kazi/releases/latest) 裡的 `kazi-<version>.apk`，傳到裝置上裝即可。

> 第一次裝可能要在系統設定打開「允許安裝來源不明的 APK」。

## 主要功能

### 影片瀏覽
- **多站聚合搜尋**：同時搜多個資源站，依片名聚合來源
- **支援排除詞**：在 token 前加 `-` 就會排除（例如 `-第二季 -預告` 可排除掉預告／續集）
- **繁簡轉換**：搜尋框旁邊一鍵把關鍵字轉簡體
- **狀態保留**：返回時搜尋結果、選中的站台、影片捲動位置都還在

### 看片體驗
- **支援 HLS / DASH**，畫面會依影片比例自動旋轉橫直
- **手機操作**（YouTube 風）：
  - 單擊：顯示／隱藏控制列
  - 雙擊左 1/3：往回 10 秒（連按可累加）
  - 雙擊右 1/3：往前 10 秒（連按可累加）
  - 雙擊中央：暫停
  - 長按：暫時 2 倍速（鬆手回原本速度）
  - 拖進度條：上方會顯示時間 tooltip
  - 垂直滑左半：調亮度
  - 垂直滑右半：調音量
- **電視盒操作**：
  - OK：暫停
  - ←／→：快轉（單按 10 秒，按住會二次方加速：1→11s、5→25s、10→40s、20→110s）
  - ↑／↓：切換倍速
  - 頻道±：切上／下集
- **橫式自動全螢幕**（手機橫過來就藏掉狀態列做真全螢幕）

### 觀看歷史 / 我的收藏
- 看片自動記住進度，下次點同一集從上次的時間點繼續
- 收藏喜歡的影片放在「收藏」區；看過的收藏卡會直接顯示「看到第幾集 + 進度」，點下去接續上次
- 歷史頁可以一鍵檢查所有片是不是有更新（多了新集數會標記）

### 帳號同步（跟網頁版 / 其他裝置互通）
搭配網頁版 [maccms-parser](https://github.com/pp771007/maccms-parser) 用同一組帳號，觀看歷史 + 收藏就能跨裝置同步（手機、電視盒、網頁互通）。

**怎麼設定**：「設定 → 帳號同步」填網頁版網址 + 密碼，按「儲存並測試連線」即可（網址貼根目錄或子頁都行，會自動解析）。

- 已綁定會顯示「已綁定：暱稱」與「最後同步」時間；要換帳號按「解除綁定」
- **刪除也會同步**：在任何一端刪掉歷史 / 收藏，其他裝置同步後也會跟著消失

### 無痕模式
首頁右上角的「🚫」按鈕點一下就開啟。開啟後：
- 搜尋不會留紀錄
- 看片不會寫進觀看歷史

設定會記住，App 重開維持上次的狀態。

### 遠端遙控
電視盒上用遙控器打字很痛苦，這個功能讓你用手機操控 TV：

**怎麼開**：首頁右上角的「QR Code」icon 按一下，會自動啟用並顯示 QR Code。

**手機這邊**：
1. 跟 TV 連同個 WiFi
2. 用手機相機掃 QR（最快），或看 TV 螢幕上的網址手動輸入
3. 手機瀏覽器會打開一個有四個分頁的網頁

**四個分頁**：
- **📱 遠端搜尋**：手機打字、勾要搜的站，送出後 TV 螢幕會自動跳到搜尋結果
- **📥 匯入站點**：把另一台裝置的站點設定快速搬過來（在原裝置點「匯出站點到剪貼簿」→ 在這頁貼上 → 預覽 → 匯入）
- **⚙️ 站點管理**：新增 / 啟停 / 排序 / 刪除站台
- **🔄 同步**：用手機打字設定帳號同步的網址 + 密碼（也可以直接在「設定 → 帳號同步」設，這個是給用遙控器不方便打字時用）

**怎麼關**：
- 首頁標題「咔滋影院」旁邊會出現一顆綠色「🟢 遠端遙控」膠囊，點一下就關
- 或回到 QR 那頁按「停止遠端遙控」

⚠️ **沒有密碼保護**，同 WiFi 上的人只要拿到網址都能進來操控。**只建議在自家 WiFi 用**。

## 站點怎麼來

App 不內建任何站台 — 你需要自己提供 MacCMS 規格的 API URL（通常長得像 `https://example.com/api.php/provide/vod/`）。

「設定」→「站點管理」可以：
- 手動貼一個 URL 新增
- 「掃描站台」用 Google 搜尋自動找一批候選

## 反饋

Bug / 建議到 [Issues](https://github.com/pp771007/kazi/issues)。
